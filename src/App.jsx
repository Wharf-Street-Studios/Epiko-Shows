...
const App = () => {
    ...
    return (
      <div className="min-h-full bg-gray-950 flex flex-col">
         <div className="p-4 flex items-center gap-4 border-b border-gray-900 bg-gray-950 sticky top-0 z-10">
            <Button variant="ghost" onClick={() => navigate('home')} className="text-white p-2 -ml-2 hover:bg-gray-900"><ChevronLeft /></Button>
            <h2 className="text-white font-bold text-lg">{filter === 'All' ? 'All Movies' : filter}</h2>
         </div>
         <div className="p-4 grid grid-cols-2 gap-4">
            {filtered.map(movie => (
               <Card key={movie.id} onClick={() => { setSelectedMovie(movie); navigate('movie-details'); }} className="cursor-pointer bg-transparent border-none shadow-none p-0">
    ...
    return (
      <div className="min-h-full bg-gray-950 flex flex-col p-6">
        <div className="flex items-center gap-4 mb-8">
          <Button variant="ghost" onClick={() => navigate('profile')} className="text-white p-2 hover:bg-gray-900 rounded-full"><ChevronLeft className="w-5 h-5" /></Button>
        ...
        <div className="space-y-6">
            <div className="space-y-2">
                ...
                <Input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="bg-gray-900 border-gray-800 text-white focus-visible:ring-red-500" />
            </div>
            <div className="space-y-2">
                <label className="text-gray-400 text-[10px] uppercase font-bold tracking-wider">Email Address</label>
                <Input type="email" value={formData.email} disabled className="bg-gray-900/50 border-gray-800/50 text-gray-500 cursor-not-allowed" />
            </div>
            <div className="space-y-2">
                <label className="text-gray-400 text-[10px] uppercase font-bold tracking-wider">Phone Number</label>
                <Input type="tel" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} className="bg-gray-900 border-gray-800 text-white focus-visible:ring-red-500" />
            </div>
        </div>
        <Button onClick={handleSave} className="mt-auto w-full bg-red-600 hover:bg-red-500 text-white py-6 font-bold text-lg shadow-lg shadow-red-900/20">Save Changes</Button>
      </div>
    );
  };
        ...
        <div className="space-y-3 overflow-y-auto pb-10">
            {[{ title: "Free Popcorn (Regular)", points: 500, icon: "🍿" }, { title: "₹100 Off Voucher", points: 1000, icon: "🎟️" }].map((r, i) => (
                <div key={i} className="bg-gray-900 p-4 rounded-xl border border-gray-800 flex justify-between items-center">
                    ...
                    <Button disabled={loyaltyPoints < r.points} onClick={() => showToast(`Redeemed: ${r.title}`)} className={`px-4 py-2 rounded-lg text-xs font-bold ${loyaltyPoints >= r.points ? 'bg-white text-black hover:bg-gray-200' : 'bg-gray-800 text-gray-600'}`}>Redeem</Button>
                </div>
            ))}
        </div>
      </div>
        ...
        <div className="space-y-4">
            {[{ code: "WELCOME50", desc: "Get 50% off on your first booking upto ₹150", valid: "Valid till 30 Dec" }, { code: "BLOCKBUSTER", desc: "Flat ₹100 off on IMAX shows", valid: "Valid till Sun" }].map((v, i) => (
                <div key={i} className="bg-white text-black p-5 rounded-2xl relative overflow-hidden shadow-lg">
                    ...
                    <Button variant="ghost" onClick={() => showToast('Code copied!')} className="mt-4 w-full py-3 border-t border-dashed border-gray-300 font-bold text-xs uppercase hover:bg-gray-50 text-gray-800 rounded-none">Tap to Copy Code</Button>
                </div>
            ))}
        </div>
    </div>
  ...
  const LoginScreen = () => {
    ...
    <div className="flex flex-col items-center justify-center h-full px-6 bg-gray-950 text-white relative overflow-hidden">
      ...
      <Card className="w-full max-w-sm bg-gray-900/50 border-gray-800 backdrop-blur-sm z-10">
        <CardContent className="pt-6 space-y-4">
            <div className="space-y-3">
                <Input type="email" placeholder="Email Address" value={email} onChange={(e) => setEmail(e.target.value)} className="bg-gray-900 border-gray-800 text-white focus-visible:ring-red-500" />
                <Input type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} className="bg-gray-900 border-gray-800 text-white focus-visible:ring-red-500" />
                <div className="flex justify-end"><Button variant="link" onClick={() => navigate('forgot-password')} className="text-xs text-gray-500 hover:text-white p-0 h-auto">Forgot Password?</Button></div>
            </div>
            <Button onClick={handleLogin} className="w-full bg-red-600 hover:bg-red-500 text-white font-bold shadow-lg shadow-red-900/20">Login</Button>
            ...
            <div className="grid grid-cols-2 gap-3">
                <Button variant="outline" onClick={loginUser} className="bg-gray-900 border-gray-800 hover:bg-gray-800 text-white gap-2"><img src="https://www.svgrepo.com/show/475656/google-color.svg" className="w-5 h-5" alt="G" /><span className="text-sm font-bold">Google</span></Button>
                <Button variant="outline" onClick={loginUser} className="bg-gray-900 border-gray-800 hover:bg-gray-800 text-white gap-2"><svg className="w-5 h-5 fill-white" viewBox="0 0 24 24"><path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.74 1.18 0 2.45-1.62 4.37-1.54 1.85.08 3.2.76 4.08 1.97-3.61 1.92-3 6.45.54 7.78-.62 1.62-1.59 3.31-3.08 4.02zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z"></path></svg><span className="text-sm font-bold">Apple</span></Button>
            </div>
        </CardContent>
      </Card>
      <p className="mt-8 text-sm text-gray-500">Don't have an account? <Button variant="link" onClick={() => navigate('signup')} className="text-red-500 font-bold p-0 h-auto">Sign Up</Button></p>
    </div>
    );
  };
        ...
        <div className="flex flex-col items-center justify-center h-full px-6 bg-gray-950 text-white relative overflow-hidden">
            <div className="w-full max-w-sm z-10">
                ...
                <Card className="bg-gray-900/50 border-gray-800 backdrop-blur-sm">
                    <CardContent className="pt-6 space-y-4">
                        <div className="space-y-3">
                            <Input type="text" placeholder="Full Name" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="bg-gray-900 border-gray-800 text-white focus-visible:ring-red-500" />
                            <Input type="email" placeholder="Email Address" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="bg-gray-900 border-gray-800 text-white focus-visible:ring-red-500" />
                            <Input type="password" placeholder="Password" value={formData.password} onChange={(e) => setFormData({...formData, password: e.target.value})} className="bg-gray-900 border-gray-800 text-white focus-visible:ring-red-500" />
                            <Input type="password" placeholder="Confirm Password" value={formData.confirmPassword} onChange={(e) => setFormData({...formData, confirmPassword: e.target.value})} className="bg-gray-900 border-gray-800 text-white focus-visible:ring-red-500" />
                        </div>
                        <Button onClick={handleSignup} className="w-full bg-red-600 hover:bg-red-500 text-white font-bold shadow-lg shadow-red-900/20">Sign Up</Button>
                ...
                <p className="text-center text-sm text-gray-500 mt-6">Already have an account? <Button variant="link" onClick={() => navigate('login')} className="text-red-500 font-bold p-0 h-auto">Login</Button></p>
            </div>
        </div>
      );
  };
  ...
  const ForgotPasswordScreen = () => {
      ...
      return (
        <div className="flex flex-col items-center justify-center h-full px-6 bg-gray-950 text-white relative overflow-hidden">
            <div className="w-full max-w-sm z-10">
                <Button variant="ghost" onClick={() => navigate('login')} className="mb-8 text-gray-500 hover:text-white flex items-center gap-2 pl-0 hover:bg-transparent"><ChevronLeft className="w-5 h-5" /> Back to Login</Button>
                ...
                <Button onClick={handleReset} className="w-full bg-red-600 hover:bg-red-500 text-white font-bold shadow-lg shadow-red-900/20">Send Reset Link</Button>
            </div>
        </div>
      );
  };
    ...
    return (
      <div className="min-h-full pb-20 bg-gray-950">
        ...
        <div className="px-6 mb-6 mt-2">
          <div className="bg-gray-900 rounded-2xl flex items-center px-4 py-1 border border-gray-800 focus-within:border-red-500/50 transition-colors shadow-lg shadow-black/20">
            <Search className="text-gray-500 w-5 h-5" /><Input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Movies, genres, theatres..." className="bg-transparent border-none outline-none text-white ml-1 w-full placeholder-gray-600 text-sm font-medium focus-visible:ring-0 shadow-none" />
        ...
        <div className="px-6 flex gap-4 mb-6 overflow-x-auto no-scrollbar">
          {['Now Showing', 'Upcoming'].map(f => (
            <Button key={f} onClick={() => setFilter(f)} variant={filter === f ? 'default' : 'outline'} className={`rounded-full text-xs font-bold whitespace-nowrap ${filter === f ? 'bg-red-600 hover:bg-red-500 text-white shadow-lg shadow-red-600/20 border-none' : 'bg-gray-900 text-gray-400 border-gray-800 hover:bg-gray-800 hover:text-white'}`}>{f}</Button>
          ))}
        </div>
        <div className="px-6 pb-4">
          <div className="flex justify-between items-end mb-4">
             <h3 className="text-white font-bold text-lg">All Movies</h3>
             <Button variant="link" onClick={() => navigate('see-all-movies')} className="text-red-500 text-xs font-bold flex items-center p-0 h-auto hover:no-underline">See All <ChevronRight className="w-3 h-3 ml-1" /></Button>
          </div>
          <div className="grid grid-cols-2 gap-x-4 gap-y-6">
            {filteredMovies.slice(0, 4).map(movie => (
              <Card key={movie.id} onClick={() => { setSelectedMovie(movie); navigate('movie-details'); }} className="group cursor-pointer bg-transparent border-none shadow-none p-0">
  ...
  const MovieDetailsScreen = () => (
    <div className="min-h-full flex flex-col bg-gray-950 relative">
      <Button variant="ghost" onClick={() => navigate('home')} className="absolute top-6 left-6 z-20 w-10 h-10 bg-black/30 backdrop-blur-md border border-white/10 rounded-full flex items-center justify-center text-white hover:bg-black/50 transition p-0"><ChevronLeft className="w-5 h-5" /></Button>
      ...
      <div className="px-6 -mt-20 relative z-10 flex-1 flex flex-col pb-6">
        ...
        <div className="flex gap-3 mb-6"><Button variant="outline" className="flex-1 bg-gray-900 border-gray-800 py-6 rounded-xl font-bold text-sm text-white flex items-center justify-center gap-2 hover:bg-gray-800 hover:text-white transition"><PlayCircle className="w-4 h-4" /> Watch Trailer</Button><Button variant="outline" onClick={() => showToast("Link copied to clipboard")} className="bg-gray-900 border-gray-800 p-3 h-auto rounded-xl text-white hover:bg-gray-800 hover:text-white transition"><Share2 className="w-4 h-4" /></Button></div>
        ...
        <Button onClick={() => { setSelectedDate(upcomingDates[0]); navigate('theatre-selection'); }} className="mt-auto w-full bg-red-600 text-white py-6 rounded-2xl font-bold text-lg shadow-xl shadow-red-900/30 hover:bg-red-500 active:scale-[0.98] transition-all">Book Tickets</Button>
      </div>
    </div>
  );
  ...
  const TheatreSelectionScreen = () => (
    <div className="min-h-full bg-gray-950 flex flex-col">
      <div className="p-4 flex items-center gap-4 border-b border-gray-900 bg-gray-950 sticky top-0 z-10">
        <Button variant="ghost" onClick={() => navigate('movie-details')} className="text-white p-2 -ml-2 hover:bg-gray-900"><ChevronLeft /></Button>
      ...
      <div className="flex-1">
        ...
        <div className="px-4 space-y-4 pb-6">
            ...
            <Card key={theatre.id} className="bg-gray-900 rounded-2xl p-5 border border-gray-800 hover:border-gray-700 transition">
                ...
                <div className="grid grid-cols-3 gap-3">{theatre.times.map(time => (<Button variant="outline" key={time} onClick={() => { setSelectedTheatre(theatre); setSelectedTime(time); setSelectedSeats([]); navigate('seat-selection'); }} className="py-2.5 border-gray-700 rounded-xl text-white text-xs font-medium bg-transparent hover:bg-gray-800 hover:text-white hover:border-gray-500 transition active:scale-95">{time}</Button>))}</div>
            </Card>
            ))}
        </div>
      </div>
    ...
    return (
      <div className="min-h-full bg-gray-950 flex flex-col">
        <div className="p-4 flex justify-between items-center bg-gray-950 z-10 border-b border-gray-900">
            <div className="flex items-center gap-3"><Button variant="ghost" onClick={() => navigate('theatre-selection')} className="text-white p-1 hover:bg-gray-900"><ChevronLeft /></Button><div><h2 className="text-white font-bold text-sm">{selectedTheatre.name}</h2><p className="text-gray-500 text-[10px]">{selectedDate.fullLabel}, {selectedDate.day} • {selectedTime}</p></div></div>
        ...
        <div className="bg-gray-900 p-5 rounded-t-3xl border-t border-gray-800 shadow-2xl sticky bottom-0 z-20">
          ...
          <Button disabled={selectedSeats.length === 0} onClick={() => navigate('payment')} className={`w-full py-6 rounded-xl font-bold text-lg transition-all flex items-center justify-center gap-2 ${selectedSeats.length > 0 ? 'bg-white text-black hover:bg-gray-200 shadow-lg shadow-white/10' : 'bg-gray-800 text-gray-600 cursor-not-allowed'}`}>Proceed to Pay</Button>
        </div>
      </div>
    );
  };
  ...
  const PaymentScreen = () => {
    ...
    const total = Math.max(0, baseTotal - discount + CONVENIENCE_FEE);
    ...
    <div className="min-h-full bg-gray-950 flex flex-col">
      <div className="p-4 flex items-center gap-4 border-b border-gray-900 bg-gray-950 sticky top-0 z-10">
        <Button variant="ghost" onClick={() => navigate('seat-selection')} className="text-white p-2 -ml-2 hover:bg-gray-900"><ChevronLeft /></Button>
        <h2 className="text-white font-bold text-lg">Payment</h2>
      </div>
      <div className="p-6 flex-1">
        <Card className="bg-gray-900 rounded-2xl p-5 mb-6 border border-gray-800">
            <div className="flex gap-4 mb-4">
                <img src={selectedMovie.poster} className="w-16 h-24 rounded-lg object-cover shadow-lg" alt="poster" />
                <div><h3 className="text-white font-bold text-lg leading-tight mb-1">{selectedMovie.title}</h3><p className="text-gray-500 text-xs mb-1">{selectedTheatre.name}</p><p className="text-gray-500 text-xs">{selectedDate.fullLabel} | {selectedTime}</p></div>
            </div>
            <div className="border-t border-gray-800 pt-4 space-y-2">
                <div className="flex justify-between text-sm"><span className="text-gray-400">Seats ({selectedSeats.length})</span><span className="text-white font-medium">{selectedSeats.map(s => s.id).join(', ')}</span></div>
                <div className="flex justify-between text-sm"><span className="text-gray-400">Subtotal</span><span className="text-white font-medium">₹{baseTotal}</span></div>
                {discount > 0 && <div className="flex justify-between text-sm text-green-500"><span>Discount</span><span>- ₹{discount}</span></div>}
                <div className="flex justify-between text-sm"><span className="text-gray-400">Convenience Fee</span><span className="text-white font-medium">₹{CONVENIENCE_FEE}</span></div>
                <div className="border-t border-gray-800 pt-2 flex justify-between items-center"><span className="text-white font-bold">Total Amount</span><span className="text-red-500 font-black text-xl">₹{total}</span></div>
            </div>
        </Card>
        <div className="mb-8">
            <h3 className="text-white font-bold text-sm mb-3 uppercase tracking-wider">Promo Code</h3>
            <div className="flex gap-3">
                <Input type="text" placeholder="Enter coupon code" value={couponCode} onChange={(e) => setCouponCode(e.target.value)} className="flex-1 bg-gray-900 border-gray-800 rounded-xl p-4 text-white outline-none focus-visible:ring-red-500" />
                <Button variant="outline" onClick={applyCoupon} className="bg-gray-900 border-gray-800 text-white font-bold rounded-xl px-6 hover:bg-gray-800 hover:text-white">Apply</Button>
            </div>
        </div>
        <div className="space-y-3">
            <h3 className="text-white font-bold text-sm mb-3 uppercase tracking-wider">Payment Method</h3>
            {['UPI', 'Credit/Debit Card', 'Net Banking'].map(method => (
                <div key={method} onClick={() => setPaymentMethod(method === 'Credit/Debit Card' ? 'card' : 'wallet')} className={`bg-gray-900 p-4 rounded-xl border border-gray-800 flex justify-between items-center cursor-pointer hover:border-gray-600 transition ${paymentMethod === (method === 'Credit/Debit Card' ? 'card' : 'wallet') ? 'border-red-500 bg-red-900/10' : ''}`}><div className="flex items-center gap-3"><div className="w-10 h-6 bg-gray-800 rounded flex items-center justify-center"><CreditCard className="w-4 h-4 text-gray-500" /></div><span className="text-white font-medium text-sm">{method}</span></div><div className={`w-4 h-4 rounded-full border ${paymentMethod === (method === 'Credit/Debit Card' ? 'card' : 'wallet') ? 'bg-red-500 border-red-500' : 'border-gray-600'}`}></div></div>
            ))}
        </div>
      </div>
      <div className="p-6 bg-gray-900 border-t border-gray-800 sticky bottom-0 z-20">
        <Button onClick={handlePaymentClick} className="w-full bg-red-600 text-white py-6 rounded-xl font-bold text-lg shadow-xl shadow-red-900/30 hover:bg-red-500 active:scale-[0.98] transition-all flex items-center justify-center gap-2">Pay ₹{total}</Button>
      </div>
      ...
      {showCardModal && (
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in slide-in-from-bottom-10">
              <div className="bg-gray-900 w-full max-w-sm rounded-2xl border border-gray-800 p-6 relative">
                  ...
                  <div className="space-y-4">
                      <Input type="text" placeholder="Card Number" className="w-full bg-black border-gray-700 rounded-xl p-3 text-white outline-none focus:border-green-500" />
                      <div className="flex gap-4">
                          <Input type="text" placeholder="MM/YY" className="w-1/2 bg-black border-gray-700 rounded-xl p-3 text-white outline-none focus:border-green-500" />
                          <Input type="text" placeholder="CVV" className="w-1/2 bg-black border-gray-700 rounded-xl p-3 text-white outline-none focus:border-green-500" />
                      </div>
                      <Button onClick={processBooking} className="w-full py-3 bg-green-500 text-black font-bold rounded-xl mt-4 hover:bg-green-400">Pay Securely</Button>
                  </div>
              </div>
          </div>
      )}
  ...
  const ConfirmationScreen = () => {
    ...
    return (
    <div className="min-h-full flex flex-col items-center justify-center bg-gray-950 px-6 text-center relative overflow-hidden">
        <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/stardust.png')] opacity-10"></div>
        <div className="w-24 h-24 bg-green-500 rounded-full flex items-center justify-center mb-6 shadow-[0_0_50px_rgba(34,197,94,0.4)] animate-bounce"><Check className="w-12 h-12 text-white" /></div>
        <h2 className="text-3xl font-black text-white mb-2">Booking Confirmed!</h2>
        <p className="text-gray-400 mb-8 max-w-xs mx-auto">Your tickets have been booked successfully. Enjoy the show!</p>
        <Card className="bg-gray-900/80 backdrop-blur-sm p-6 rounded-2xl border border-gray-800 w-full max-w-sm mb-8 relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-red-500 via-purple-500 to-blue-500"></div>
            <div className="flex gap-4 items-start text-left">
                <img src={booking.movie.poster} className="w-16 h-24 rounded-lg object-cover" alt="poster" />
                <div><h3 className="text-white font-bold text-lg leading-tight mb-1">{booking.movie.title}</h3><p className="text-gray-400 text-xs mb-2">{booking.theatre.name}</p><div className="flex gap-2"><span className="bg-gray-800 text-white text-[10px] px-2 py-1 rounded font-bold">{booking.time}</span><span className="bg-gray-800 text-white text-[10px] px-2 py-1 rounded font-bold">{booking.displayDate || booking.date}</span></div></div>
            </div>
            <div className="mt-4 pt-4 border-t border-gray-800 flex justify-between items-center"><div className="text-left"><p className="text-gray-500 text-[10px] uppercase font-bold">Seats</p><p className="text-white font-bold">{booking.seats.map(s => s.id).join(', ')}</p></div><div className="text-right"><p className="text-gray-500 text-[10px] uppercase font-bold">Order ID</p><p className="text-white font-mono text-xs">#{booking.id}</p></div></div>
        </Card>
        <div className="flex flex-col gap-3 w-full max-w-sm">
            <Button onClick={() => navigate('home')} className="w-full bg-gray-800 text-white py-6 rounded-xl font-bold hover:bg-gray-700 transition">View Ticket</Button>
            <Button variant="ghost" onClick={() => navigate('home')} className="w-full text-gray-500 hover:text-white py-6 hover:bg-transparent">Back to Home</Button>
        </div>
    </div>
    );
  };
    ...
    return (
        <div className="min-h-full bg-gray-950 flex flex-col">
            ...
            <div className="p-6 flex-1">
                ...
                <div className="flex gap-2 mb-4"><div className="relative flex-1"><span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 font-bold">₹</span><Input type="number" min="1" value={amountToAdd} onChange={(e) => setAmountToAdd(e.target.value)} placeholder="Enter amount" className="bg-gray-900 border-gray-700 text-white pl-8 focus-visible:ring-red-500" /></div><Button onClick={handleAddFunds} disabled={!amountToAdd} className="bg-white text-black px-6 font-bold hover:bg-gray-200 disabled:opacity-50">Add</Button></div>
                <div className="flex gap-2 mb-8 overflow-x-auto no-scrollbar">{quickAdd.map(amt => (<Button key={amt} variant="outline" onClick={() => setAmountToAdd(amt.toString())} className="px-4 py-2 bg-gray-900 border-gray-800 text-gray-400 text-xs font-bold hover:bg-gray-800 hover:text-white whitespace-nowrap">+ ₹{amt}</Button>))}</div>
  ...
  const BookingsScreen = () => {
    ...
    return (
        <div className="min-h-full bg-gray-950 flex flex-col relative">
             <div className="p-6 sticky top-0 bg-gray-950 z-10"><h2 className="text-white font-black text-2xl mb-6">My Bookings</h2><div className="flex bg-gray-900 p-1 rounded-xl">{['Upcoming', 'Past'].map(t => (<Button key={t} variant="ghost" onClick={() => setTab(t)} className={`flex-1 py-2 rounded-lg text-xs font-bold ${tab === t ? 'bg-gray-800 text-white shadow-md' : 'text-gray-500 hover:bg-transparent hover:text-white'}`}>{t}</Button>))}</div></div>
                ...
                {filtered.length === 0 ? (<div className="text-center py-20 opacity-50"><Ticket className="w-16 h-16 text-gray-700 mx-auto mb-4" /><p className="text-gray-500">No {tab.toLowerCase()} bookings</p></div>) : (
                    filtered.map(b => (
                        <div key={b.id} className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden shadow-xl">
                            ...
                            <div className="bg-gray-900 p-3 flex justify-between items-center"><div className="text-xs text-gray-400">{b.seats.length} Seats: <span className="text-white font-bold">{b.seats.map(s=>s.id).join(', ')}</span></div><Button variant="outline" onClick={() => setSelectedQr(b)} className="text-red-500 text-xs font-bold border-red-900/30 px-3 py-1.5 rounded-lg hover:bg-red-900/10 bg-transparent">View QR</Button></div>
                        </div>
                    ))
                )}
             </div>
             {selectedQr && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-6 animate-in fade-in duration-200">
                    <div className="bg-white rounded-3xl w-full max-w-xs overflow-hidden relative shadow-2xl animate-in zoom-in-95 duration-300">
                        <Button variant="ghost" onClick={() => setSelectedQr(null)} className="absolute top-4 right-4 p-2 bg-gray-100 rounded-full hover:bg-gray-200 z-10"><X className="w-5 h-5 text-gray-900" /></Button>
  ...
  const ProfileScreen = () => (
      <div className="min-h-full bg-gray-950 p-6 flex flex-col">
          <div className="flex justify-between items-center mb-8"><h2 className="text-white font-black text-2xl">Profile</h2><Button variant="ghost" onClick={() => navigate('profile-settings')} className="text-gray-500 hover:text-white p-2"><Settings className="w-6 h-6" /></Button></div>
          ...
          <Button variant="ghost" onClick={() => { setUser(null); navigate('login'); }} className="mt-auto flex items-center justify-center gap-2 text-red-500 font-bold py-4 hover:bg-red-900/10 rounded-xl"><LogOut className="w-5 h-5" /> Logout</Button>
      </div>
  );
